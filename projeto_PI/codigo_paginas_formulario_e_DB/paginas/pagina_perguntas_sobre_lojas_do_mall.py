...
from config.lists_for_functions import lista_segmentos, escala_satisfacao, sim_nao, lista_0_a_10
...
def perguntas_lojas():
    ...
    st.session_state.p3_pl = st.selectbox("Como você avalia o atendimento nas lojas que visitou?", lista_0_a_10, index=lista_0_a_10.index(st.session_state.p3_pl))
    ...
    st.session_state.sm_pl = st.text_area("Quais produtos ou serviços você gostaria de ver nas lojas que ainda não estão disponíveis?", value=st.session_state.sm_pl)
